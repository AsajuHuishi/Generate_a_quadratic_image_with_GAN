...
# -*- coding:utf-8 -*-
###国码国造-试作雍宁计划
##雍宁计划目标：1.第一次改造他人代码用其他框架实现(Keras 改造为pytorch),剧透重要意义
###2.学习的第一个基于GAN的网络
...
def load_data():
    x_train = np.empty((25, 64, 64, 3), dtype="float32")
    images = os.listdir('./part_images/images/')

    lens = len(images)
    for i in range(lens):
        img = Image.open('./part_images/images/' + images[i])
...
def main():
    gan = GAN()
    gan.train(epochs=3000, batch_size=32, sample_interval=20)
